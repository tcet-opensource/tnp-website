...
export interface Props {
  superStarPerformers: SuperStarPerformer[];
}

const SuperStarPerformers:React.FC<Props> = ({superStarPerformers}) => {
  const Cards = superStarPerformers.map((superStarPerformer: SuperStarPerformer) => {
    return (
      <SwiperSlide key={superStarPerformer.name}>
        <div className="text-sans bg-slate-50 p-4 border-1 rounded-xl border">
          {/* <!-- Image  --> */}
          <div className="min-w-full">
            <img className="rounded-lg w-full h-40 object-cover" src={superStarPerformer.img} alt="Super Star Performer Photo" />
          </div>

          {/* <!-- Batch  --> */}
          <div className="mt-2">
            <span className="text-sm font-medium px-2 py-1 rounded-full bg-gray-200 text-gray-700">{superStarPerformer.batch}</span>
          </div>

          {/* <!-- Name  --> */}
          <div className="mt-1">
            <span className="text-lg font-semibold text-gray-900">{superStarPerformer.name}</span>
          </div>

          {/* <!-- Company  --> */}
          <div>
            <span className="text-base font-normal text-gray-600">@{superStarPerformer.company}</span>
          </div>

          {/* <!-- Branch  --> */}
          <div>
            <span className="text-base font-normal text-gray-600">{superStarPerformer.branch}</span>
          </div>

          {/* <!-- Package --> */}
          <div className="text-center rounded-lg bg-gray-200 mt-1 p-2">
            <span className="text-lg font-semibold text-gray-900">{superStarPerformer.package} Lakhs</span>
          </div>
        </div>
      </SwiperSlide>
    );
