...
const Staff: React.FC<{
  ...
  return <div className="md:flex grid grid-cols-1  md:justify-center pt-4 flex-wrap gap-8">{data.map(staff => <StaffPost {...staff}/>)}</div>;
};
...
const StaffPost = (staffInfo: StaffTNP) => {
    return (
        <div className="border bg-[#f9fafb] rounded-lg p-4 flex flex-col justify-center items-center gap-2">
            <img src={"/Staff/"+staffInfo.staffPhoto} alt="staffFoto" className="w-52 aspect-square object-cover rounded-xl"/>
